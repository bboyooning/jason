    ...
    var jason = {
        ...
        section3Fn: function () {
            ...
            function mainPrevSlideFn() {
                // console.log(cnt);
                $slide.css({zIndex:1,opacity:1}); // 초기화 모든 슬라이드 zIndex:1
            ...
            // case 0 : 현재 슬라이드가 
            // 첫번째 슬라이드 라면, 페이지 버튼 1 : (index number:[1]) 두번째 슬라이드 이미지 s3Slide1.jpg 
            // 첫번째 슬라이드 라면, 페이지 버튼 2 : (index number:[2]) 세번째 슬라이드 이미지 s3Slide2.jpg
            
            // case 1 : 현재 슬라이드가 
            // 두번째 슬라이드 라면, 페이지 버튼 1 : (index number:[0]) 첫번째 슬라이드 이미지 s3Slide0.jpg 
            // 두번째 슬라이드 라면, 페이지 버튼 2 : (index number:[2]) 세번째 슬라이드 이미지 s3Slide2.jpg 

            // case 2 : 현재 슬라이드가 
            // 세번째 슬라이드 라면, 페이지 버튼 1 : (index number:[0]) 첫번째 슬라이드 이미지 s3Slide0.jpg
            // 세번째 슬라이드 라면, 페이지 버튼 2 : (index number:[1]) 두번째 슬라이드 이미지 s3Slide1.jpg
            ...
            $pageBtn.each(function(idx){
                $(this).on({
                    click:  function(e){
                        e.preventDefault();
                        // 바뀌기 이전 상태
                        console.log('현재 슬라이드 번호', cnt); // 현재 실행중인 슬라이드 번호가 뭐니?
                        console.log('클릭한 슬라이드 번호', a[idx]); // 클릭한 슬라이드 번호


                        var imsi = cnt ;   // 현재 실행중인 슬라이드 번호를 임시에 넣어주 그리고 비교 
                            cnt  = a[idx]; // a[1,2] 배열값(인수) 불러와잉 클릭한 인수에 해당된 배열값

                            if(imsi < a[idx]){ // 클릭한 번호가 더 크면 다음 슬라이드
                                mainNextSlideFn(); // 함수 실행 범위(스코프 scope)에 변수 cnt가 할당
                            }
                            if(imsi > a[idx]){ // 클릭한 번호가 더 작으면 이전 슬라이드
                                mainPrevSlideFn();
                            }
                            // 바뀐 결과 후 변수 값들
                            console.log('현재 슬라이드 번호',cnt); // 현재 실행중인 슬라이드 번호가 뭐니?
        ...
        section4Fn: function () {            
            ...
            var totN            = $('#section4 .slide').length; // 10개
            var slideN          = 3; // 980초과 3, 980이하 2, 모바일 1
            ...
            var setId2          = null;
                ...
                ////////////////////////// 메인 슬라이드 반응형 //////////////////////////
                ////////////////////////////////////////////////////////////////////////
                // slide container 박스 너비에 따른 슬라이드 3개의 너비 구하기
                // 1570px - 40px(margin 20px*2) =1530px; 
                // 슬라이드 너비는 1570px/3 = 523.3333333px
                // 반응형으로 ('.slide-container') 박스 너비 변화에 따른 슬라이드 너비 계산 자동  
                ...
                function resizeFn(){
                    ...
                    // $slideWrap.css({width:(slideW*10), marginLeft:-(slideW*3)}); // 3 4 3 
                ...
                setTimeout(resizeFn, 10); // 처음 로딩시 딱 한번만 실행 또는 새로고침
                ...
                function timerControlFn(){
                    var tcnt = 0;
                    clearInterval(setId);  // 중지.초기화 
                    clearInterval(setId2); // 중지.초기화 
                    setId2 = setInterval(function(){
                        tcnt++;
                        if(tcnt>=6){
                            clearInterval(setId2);
                            nextCountSlideFn(); // 6초 후 즉각 실행
                            autoPlayFn(); // 자동으로 6초 후 실행
                        }
                    },1000);
                }
        },
